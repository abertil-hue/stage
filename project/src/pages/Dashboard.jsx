    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-12">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-6">
        ...
        <div className="flex flex-col items-center justify-center text-center py-4 space-y-3">
          <img 
            ...
            className="h-32 sm:h-40 w-auto object-contain drop-shadow-sm transition-all"
          />
        </div>

        {/* LIGHT DASHBOARD CONTROLS HEADER */}
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white font-semibold px-5 py-2.5 rounded-xl text-xs transition-all shadow-sm hover:shadow-md"
            >
        ...
        {loading ? (
          ...
          <div className="bg-green-100 rounded-3xl border border-dashed border-slate-300 p-12 text-center flex flex-col items-center justify-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredFormations.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="bg-zinc-100 rounded-2xl border border-slate-200/80 shadow-xs hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between group overflow-hidden"
                >
                  <div className="p-6 space-y-5">
                    {/* Header: Title & Participant Counter */}
                    <div className="flex items-start justify-between gap-3">
                      <h2 className="text-base sm:text-lg font-bold text-slate-900 capitalize leading-snug group-hover:text-emerald-700 transition-colors line-clamp-2">
                        {item.title}
                      </h2>
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-700 font-bold text-xs rounded-full border border-emerald-200/60 shrink-0 shadow-2xs">
                    ...
                    {/* Session Metadata Info Grid */}
                    <div className="space-y-3 text-xs text-slate-600">
                      {/* Trainer */}
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-slate-100 text-slate-600 rounded-xl group-hover:bg-emerald-50 group-hover:text-emerald-700 transition-colors">
                          <User size={15} />
                        </div>
                  ...
                  <div className="px-6 pb-6 pt-2">
                    <Link
                      to={`/session/${item.id}`}
                      className="w-full inline-flex items-center justify-center gap-2 font-semibold text-slate-900 py-3 px-4 rounded-xl text-black active:bg-emerald-700 transition-all duration-200 shadow-sm hover:shadow-md"
                    >
                      <Eye size={15} />
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl overflow-hidden relative border border-slate-200">
            ...
            <form onSubmit={handleCreateFormation} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs focus:outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/20 bg-slate-50"
                />
              </div>
              ...
              <div className="flex gap-3 justify-end pt-4 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  disabled={creating}
                  className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-emerald-900/20"
                >
